...
def ha(open_, high, low, close, offset=None, **kwargs):
    ...
    open_col_index = df.columns.tolist().index('HA_open')
    close_col_index = df.columns.tolist().index('HA_close')
    for i in range(1, m):
        df.iloc[i, open_col_index] = 0.5 * (df.iloc[i - 1, open_col_index] + df.iloc[i - 1, close_col_index])
